...
#read in data
data<-read.csv("data/environments/homeless.csv")
data$X<-NULL
...
#dichotomize
data[,1:(endx)]<-apply(data[,1:(endx)],2,function(x){ifelse(x<median(x),0,1)})
...
for (h in seq_along(props)){
  ...
  for (i in 1:times){
    ...
    #TTB_ABC
    ##Fit model
    mabc<-ttbabcfit(X=X_learn, y=y_learn, epsilon=0.7, nsamples=100, nsims=500)
  ...
  #save on the go
  write.csv(dfinal, "data/homelesscross.csv")
}
#END