...
#read in data
data<-read.csv("data/environments/profsalary.csv")
data$X<-NULL
data$completeness_of_data<-NULL
data<-na.omit(data)
...
#dichotomize
data[,1:(endx-2)]<-apply(data[,1:(endx-2)],2,function(x){ifelse(x<median(x),0,1)})
...
for (h in seq_along(props)){
  ...
  for (i in 1:times){
    ...
    #TTB_ABC
    ##Fit model
    mabc<-ttbabcfit(X=X_learn, y=y_learn, epsilon=0.8, nsamples=100, nsims=500)
  ...
  #save on the go
  write.csv(dfinal, "data/salarycross.csv")
}
#END