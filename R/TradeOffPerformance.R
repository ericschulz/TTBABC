...
#Trading off performance by adjusting nsims, proportion, and epsilon
...
#Result of script:
##performance of ttb-abc, averaged over 30 runs
...
#Load packages
packages <- c('plyr', 'gridExtra', 'lattice')
    ...
    apply(m,1, function(y){
      ...
      mean(
        ...
        ttbabcpredict(
          ...
          ttbabcfit(xttb, yttb, nsims=x, epsilon = y[1], proportion=y[2], progress = FALSE),
          XX=xttb)$class==yttb)
      })
    })
  mcollect<-mcollect+(1/30)*matrix(unlist(performance), nrow=81)
print(mcollect)
}
#create data frame
dp<-data.frame(mcollect, m)
#name it
names(dp)<-c(paste0("y",1:4), paste0("x",1:2))

#save for plotting
write.csv(dp, "data/performancetradeoff.csv")
...
#Saving plot
pdf("figs/performancetradeoff.pdf", width=8, height=8)
#bringing it all toegther in one plot
grid.arrange(w1,w2,w3,w4, 
             ncol=2,
             main=textGrob("Trading off performance", gp=gpar(fontsize=18), vjust=0.7)
)
dev.off()
#END